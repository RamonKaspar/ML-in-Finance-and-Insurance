...
Goal: Study the dynamics of a NN $F_\theta : \R^{N_0} \to \R$ during GD.

\begin{subbox}{Definition of NTK}
For a neural network $ F_{\theta} : \mathbb{R}^{N_0} \rightarrow \mathbb{R} $, the \textbf{Neural Tangent Kernel (NTK)} $ K(x, x'; \theta) $ is:
$$
K(x, x'; \theta) = D_{\theta} F_{\theta}(x) \cdot D_{\theta} F_{\theta}(x')^T
$$
$\Rightarrow$ \textit{Measures similarity} of gradients of $ F_{\theta} $ at points $ x, x' $, determining how changes in parameters affect function output similarity.
\end{subbox}

\begin{subbox}{Convergence at Initialization}
In the \textbf{infinite-width limit} (width of hidden layers $ \rightarrow \infty $):
1. $ K(\cdot, \cdot; \theta) $ becomes deterministic and \textit{remains constant during training}.
2. $ F_{\theta}(x) $ approximates a \textit{Gaussian Process} (GP) with covariance kernel depending on initialization and activation function (Jacot et al., 2018).
\end{subbox}

\textbf{Consequence}: If $ K(\cdot, \cdot; \theta) $ is constant, gradient flow dynamics imply convergence of the loss to a global minimum.

---

\section{Generalization in Overparameterized Networks}

\textbf{The Generalization Puzzle}: Overparameterized networks (high parameter count relative to data) generalize well despite fitting data perfectly (even on noisy or random labels), defying traditional bias-variance logic:
1. Large NNs achieve near-zero \textit{training error} yet retain strong generalization.
2. Increased model size often reduces \textit{test error}, contrary to overfitting expectations.

\begin{subbox}{Explanations}
    1. \textbf{Implicit regularization}: SGD favors "simple" solutions with low effective complexity.
    2. \textbf{Minimum-norm interpolation}: Solution minimizes norm in RKHS induced by NTK, improving generalization (Neyshabur et al., 2017).
    3. \textbf{Lazy training}: In very wide NNs, parameter updates are small, and training behaves linearly.
\end{subbox}

---

\section{Gaussian Processes (GPs) and Neural Networks}

\begin{subbox}{Gaussian Process (GP) Definition}
A family of random variables $\{f(x)\}_{x \in X}$ is a \textbf{Gaussian Process} if any finite subset $(f(x_1), \dots, f(x_m))$ is multivariate Gaussian, characterized by:
1. \textbf{Mean function} $ \mu(x) = \mathbb{E}[f(x)] $
2. \textbf{Covariance kernel} $ C(x, x') = \text{Cov}(f(x), f(x')) $
\end{subbox}

\begin{subbox}{GP Interpretation of NNs}
As width $ \rightarrow \infty $, shallow NNs approximate GPs with:
$$
\text{Cov}(F_{\theta}(x), F_{\theta}(x')) = C(x, x') = \mathbb{E}\left[ \rho(f(x)) \rho(f(x')) \right] + \beta^2
$$
where $ \rho $ is the activation function.
\end{subbox}
...
\textbf{Double Descent}: Test error decreases even after reaching zero training error, often seen in large models. Characterized by:
- \textbf{Classical regime}: $ m/p \ll 1 $: Overfitting increases test error.
- \textbf{Overparameterized regime}: $ m/p \approx 1 $: Decreased test error despite complex model.

\begin{subbox}{Marchenko-Pastur Law (Random Matrix Theory)}
For large $ m, p $ with $ p/m \rightarrow \gamma $, the \textbf{spectral distribution} of a random matrix follows the Marchenko-Pastur law:
$$
\mu_{\text{MP}}(ds) = \frac{\sqrt{(\gamma_+ - s)(s - \gamma_-)}}{2\pi \sigma^2 \gamma s} 1_{[\gamma_-, \gamma_+]}(s) ds
$$
where $ \gamma\_{\pm} = \sigma^2(1 \pm \sqrt{\gamma})^2 $.
\end{subbox}
